  ...
  <div class="container">
    <div class="row align-items-center">
      <div class="col-md-13 text-center text-md-start mb-4 mb-md-0">
        ...
        <p class="lead">Dapoer MJ adalah layanan katering rumahan terpercaya yang menyajikan aneka menu dengan cita rasa rumahan yang menggugah selera. Kami menghadirkan pilihan paket seperti nasi kotak, snack box, tumpeng, nasi liwet, hingga prasmanan — semuanya dimasak dari bahan segar, porsi terukur, dan dikemas secara higienis. Pesan menu favoritmu sekarang!</p>
...
<br>
        <div class="d-flex justify-content-around flex-wrap p-4 rounded shadow-sm" style="background-color: #FEBA17; color:#4E1F00">
            ...
            <div class="row row-cols-1 row-cols-md-4 g-4">
                ...
                <div class="col">
                    <div class="card h-100 shadow-sm" style="background-color: white;">
                        <img src="{{ asset('image/' . $menu->photos->first()->photo) }}" class="card-img-top" alt="{{ $menu->name }}">
                        <div class="card-body">
                            <h5 class="card-title">{{ $menu->name }}</h5>
                            {{-- <p class="card-text fw-bold text-danger">Rp {{ number_format($menu->price, 0, ',', '.') }}</p> --}}
                            {{-- <p class="card-text"><small class="text-muted">Tersedia: {{ \Carbon\Carbon::parse($menu->date)->format('d M Y') }}</small></p> --}}
                            {{-- @if ($menu->is_popular)
                                <span class="badge bg-warning text-dark">🔥 Populer</span>
                            @endif --}}
                        </div>
                        <div class="card-footer" style="background-color: white; border: none;">
                            <a href="{{ route('menus.show', $menu) }}" style="background-color: #FEBA17" class="btn btn-warning w-100 text-white">Lihat</a>
                        </div>
                    </div>
                </div>
                @endforeach
...
<div class="mt-5" id="kontak">
    ...
    <div class="row g-4">
        ...
        <div class="col-md-6">
            ...
            <form onsubmit="return kirimWhatsApp();" class="shadow-sm p-4 rounded" style="background-color: #ffffff;">
                <div class="mb-3">
